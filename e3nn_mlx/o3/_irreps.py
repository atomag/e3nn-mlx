...
class Irrep(tuple):
    ...
    def D_from_angles(self, alpha, beta, gamma, k=None) -> mx.array:
        ...
        gamma : `mlx.core.array`
            tensor of shape :math:`(...)`
            Rotation :math:`\gamma` around Y axis, applied first.

        k : `mlx.core.array`, optional
            tensor of shape :math:`(...)`
        ...
        """
        if k is None:
            k = mx.zeros_like(alpha)

        # MLX doesn't have broadcast_tensors like PyTorch, but we can handle broadcasting differently
        alpha, beta, gamma, k = mx.broadcast_arrays(alpha, beta, gamma, k)
        return _wigner.wigner_D(self.l, alpha, beta, gamma) * (self.p ** k)[..., None, None]

    def D_from_quaternion(self, q, k=None) -> mx.array:
    ...
    def D_from_matrix(self, R) -> mx.array:
        r"""Matrix of the representation, see `Irrep.D_from_angles`
        ...
        Examples
        --------
        >>> m = Irrep(1, -1).D_from_matrix(-mx.eye(3))
        >>> m.astype(mx.int64)
        array([[-1,  0,  0],
               [ 0, -1,  0],
               [ 0,  0, -1]], dtype=int64)
        """
        d = mx.sign(mx.linalg.det(R))
        R = d[..., None, None] * R
        k = (1 - d) / 2
        return self.D_from_angles(*_rotation.matrix_to_angles(R), k)

    def D_from_axis_angle(self, axis, angle) -> mx.array:
        r"""Matrix of the representation, see `Irrep.D_from_angles`
    ...
    def __mul__(self, other):
        ...
        Returns
        -------
        generator of `e3nn_mlx.o3.Irrep`
        """
        other = Irrep(other)
        p = self.p * other.p
    ...
    def count(self, value):
        """Count occurrences of value in the tuple."""
        return super().count(value)

    def index(self, value):
        """Find index of value in the tuple."""
...
class Irreps(tuple):
    ...
    def randn(
        self, *size: int, normalization: str = "component", key=None, dtype=mx.float32
    ) -> mx.array:
        r"""Random tensor.
        ...
        *size : list of int
        ...
        key : `mlx.core.random.key`, optional
            random key for reproducibility

        dtype : `mlx.core.dtype`, optional
        ...
        >>> mx.abs(mx.linalg.norm(random_tensor, axis=1) - 1).max() < 1e-5
        True
        """
        di = size.index(-1)
        lsize = size[:di]
        ...
        if normalization == "component":
            return mx.random.normal((*lsize, self.dim, *rsize), key=key, dtype=dtype)
        elif normalization == "norm":
            x = mx.random.normal((*lsize, self.dim, *rsize), key=key, dtype=dtype)
            # Normalize along the appropriate axis
            norm = mx.linalg.norm(x, axis=di, keepdims=True)
            x = x / norm
            return x
        else:
    ...
    def index(self, obj):
        """Find index of obj in the irreps."""
        for i, (mul, ir) in enumerate(self):
            if (mul, ir) == obj:
                return i
        raise ValueError(f"{obj} not in irreps")
    ...
    def D_from_matrix(self, R):
        ...
        `mlx.core.array`
            tensor of shape :math:`(..., \mathrm{dim}, \mathrm{dim})`
        """
        # Custom determinant for 3x3 matrices since MLX doesn't have linalg.det on GPU
        def det_3x3(R):
            """Compute determinant of 3x3 matrices"""
            if R.ndim == 2 and R.shape == (3, 3):
                ...
                for i in range(R.shape[0]):
                    dets.append(det_3x3(R[i]))
                return mx.array(dets)
            else:
                # For other cases, use a simple approximation or raise error
                return mx.ones(R.shape[:-2])
