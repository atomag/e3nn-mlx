...
from e3nn_mlx.o3._irreps import Irrep, Irreps
...
class ElementwiseTensorProduct(TensorProduct):
    ...
    filter_ir_out : iterator of `e3nn_mlx.o3.Irrep`, optional
        filter to select only specific `e3nn_mlx.o3.Irrep` of the output
    ...
    def __init__(self, irreps_in1, irreps_in2, filter_ir_out=None, irrep_normalization: str = None, **kwargs) -> None:
        ...
        if filter_ir_out is not None:
            try:
                ...
                raise ValueError(f"filter_ir_out (={filter_ir_out}) must be an iterable of e3nn_mlx.o3.Irrep")
...
class TensorSquare(TensorProduct):
    ...
    irreps_out : `e3nn_mlx.o3.Irreps`, optional
        representation of the output
    ...
    def __call__(self, x, weight: Optional[mx.array] = None):  # pylint: disable=arguments-differ
