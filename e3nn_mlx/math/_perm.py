...
def is_perm(p: TY_PERM):
    """Check if a tuple is a valid permutation."""
...
def from_int(i: int, n: int) -> TY_PERM:
    """Convert an integer to a permutation."""
    pool = list(range(n))
    p = []
    for _ in range(n):
        j = i % n
...
def standard_representation(
    p: TY_PERM, dtype: Optional[mx.Dtype] = None
) -> mx.array:
    r"""irrep of Sn of dimension n - 1"""
    from ._complete_basis import complete_basis
    A = complete_basis(mx.ones((1, len(p)), dtype=dtype), eps=0.1 / len(p))
